  ...
  <div class="page-farmer-list">
    ...
    <div ref="cardList" class="card-list" @touchstart="touchStart($event)" @touchend="touchEnd($event)">
      <div class="content" v-for="(item, index) in list" :key=index>
        <div class="line-top">
          <div class="name">{{item.servicePointName}}<span>{{item.connectPhone}}</span></div>
          <div class="status">正在跟进</div>
        </div>
        <div class="line-middle">
          <div class="left">
            <div class="title">经营产品：{{item.mainProduct}}</div>
            <div class="address">服务区域：{{item.serviceArea}}</div>
          </div>
          <div class="right">
            <div class="icon"></div>
  ...
  methods: {
    ...
    urls() {
      return {
        ...
        init: `http://thegisguy.cn:8085/system/point/list`
      }
    }
  }
}
